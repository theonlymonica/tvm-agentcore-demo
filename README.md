...
This is the **token vending machine** pattern — a request-time broker that mints
narrowly scoped, short-lived credentials instead of handing out standing access —
applied to agent tools rather than to the mobile and IoT clients it is usually
associated with. The model is the untrusted client here.
